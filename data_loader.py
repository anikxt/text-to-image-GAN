...
def save_caption_vectors_ms_coco(data_dir, split, batch_size):
    ...
    while batch_no*batch_size < len(ic_data['annotations']):
        ...
        h5f_tv_batch = h5py.File(
            join(data_dir, 'tvs/'+split + '_tvs_' + str(batch_no)), 'w')
    ...
    for class_dir in class_dirs:
        ...
        for cap_file in caption_files:
            ...
            image_captions[img_file] += [
                cap for cap in captions if len(cap) > 0][0:5]
...
def main():
    ...
    if args.data_set == 'flowers':
        ...
        save_caption_vectors_ms_coco(
            args.data_dir, args.split, args.batch_size)
